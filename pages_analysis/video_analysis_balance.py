# pages_analysis/video_analysis_balance.py
...
def render(go_to):
    st.title("📊 외발서기 영상 분석 결과")

    uploaded_file = st.session_state.get("uploaded_video", None)

    if uploaded_file is None:
        st.error("먼저 외발서기 튜토리얼에서 영상을 업로드해주세요.")
        if st.button("튜토리얼로 이동"):
            go_to("tutorial_balance")
        return
