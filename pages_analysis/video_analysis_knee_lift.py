# pages_analysis/video_analysis_knee_lift.py
...
def render(go_to):
    st.title("📊 제자리 무릎들기 영상 분석 결과")

    uploaded_file = st.session_state.get("uploaded_video", None)

    if uploaded_file is None:
        st.error("먼저 제자리 무릎들기 튜토리얼에서 영상을 업로드해주세요.")
        if st.button("튜토리얼로 이동"):
            go_to("tutorial_knee_lift")
        return
    ...
    st.subheader("📌 제자리 무릎들기 자동 분석 결과")
    ...
    if st.button("제자리 무릎들기 튜토리얼로 돌아가기"):
        go_to("tutorial_knee_lift")
