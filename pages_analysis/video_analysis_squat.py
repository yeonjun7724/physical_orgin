# pages_analysis/video_analysis_squat.py
...
def render(go_to):
    st.title("📊 스쿼트 리듬 영상 분석 결과")

    uploaded_file = st.session_state.get("uploaded_video", None)

    if uploaded_file is None:
        st.error("먼저 스쿼트 리듬 튜토리얼에서 영상을 업로드해주세요.")
        if st.button("튜토리얼로 이동"):
            go_to("tutorial_squat")
        return
    ...
    if st.button("스쿼트 리듬 튜토리얼로 돌아가기"):
        go_to("tutorial_squat")
