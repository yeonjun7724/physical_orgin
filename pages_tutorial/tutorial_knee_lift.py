...
def render(go_to):
    ...
    with col_right:
        ...
        if uploaded_file is not None:
            if st.button("이 영상으로 자세 분석하기", type="primary", use_container_width=True, key="analyze_knee_lift"):
                ...
                # ⭐ 이동 경로 (중요!)
                # -------------------------
                go_to("video_analysis_knee_lift")
        else:
            st.button("이 영상으로 자세 분석하기", type="secondary", use_container_width=True, key="analyze_knee_lift", disabled=True)
