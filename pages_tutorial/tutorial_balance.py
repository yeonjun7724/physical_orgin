...
def render(go_to):
    ...
    with col_left:
        ...
        with section_card("올바른 자세", icon="✅", variant="primary"):
            ...
            youtube_url = "https://www.youtube.com/embed/vj5G3TX0xJI"
            st.markdown(f"""
    ...
    with col_right:
        ...
        if uploaded_file is not None:
            if st.button("이 영상으로 자세 분석하기", type="primary", use_container_width=True, key="analyze_balance"):
                ...
                # ⭐ 이동 경로 (중요!)
                # -------------------------
                go_to("video_analysis_balance")
        else:
            st.button("이 영상으로 자세 분석하기", type="secondary", use_container_width=True, key="analyze_balance", disabled=True)
